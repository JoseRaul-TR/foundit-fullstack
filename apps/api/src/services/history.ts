/**
 * apps/api/src/services/history.ts  (tickets #49 + #50)
 *
...
import type { PaginatedResponse } from "@foundit/types";
...
const PAGE_SIZE = 20;
const LANGUAGE = "en-US"; // no ?lang= support in either ticket
...
async function enrichMovieRow(
  ...
  const raw = await fetchTmdb<TmdbMovie>(`/movie/${row.tmdbId}`, {
    language: LANGUAGE,
  });
