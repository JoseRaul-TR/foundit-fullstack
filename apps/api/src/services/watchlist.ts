// apps/api/src/services/watchlist.ts
...
import { toSeriesStatus } from "@/services/series";
...
const PAGE_SIZE = 20;
const BASIC_LANGUAGE = "en-US"; // no ?lang= in this ticket; see note in routes/watchlist.ts
...
async function fetchRawTmdb(
  tmdbId: number,
  mediaType: MediaType,
): Promise<TmdbMovie | TmdbSeries> {
  return mediaType === "movie"
    ? fetchTmdb<TmdbMovie>(`/movie/${tmdbId}`, {
        append_to_response: "watch/providers",
        language: BASIC_LANGUAGE,
      })
    : fetchTmdb<TmdbSeries>(`/tv/${tmdbId}`, {
        append_to_response: "watch/providers",
        language: BASIC_LANGUAGE,
      });
}
...
function extractTitleAndYear(
  ...
  if (mediaType === "movie") {
    ...
    return { title: movie.title, year: parseYear(movie.release_date) };
  }
  const series = raw as TmdbSeries;
  return { title: series.name, year: parseYear(series.first_air_date) };
}
...
function buildTmdbInfoAndHighlight(
  ...
  const { title, year } = extractTitleAndYear(mediaType, raw);
...
export async function addToWatchlist(
  ...
  const { title, year } = extractTitleAndYear(input.mediaType, raw);
